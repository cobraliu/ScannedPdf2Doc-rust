...
/// 这行够不够长, 能看出是中文还是外文
///
/// `is_zh` 要两个汉字才算中文, 于是段落最后那行 "期。" 被判成"不是中文", 撞上
/// "中英切换 -> 对照的另一半"这条, 从中文段落里甩出去自成一段 —— 再短一点、
/// 置信度再低一点, 它就被 drop_noise 当印记删掉, 整行没了。一两个字本来就
/// 判不出语言, 别判: 真正的中英对照, 另一半不会只有一个字。
fn lang_clear(t: &str) -> bool {
...
pub fn merge_paras(lines: &[Line], cfg: &Config) -> Vec<Para> {
    ...
    for ln in lines {
        ...
        // 首行缩进: 中文段落第一行缩两个字, 续行退回左边界 —— 跟悬挂缩进正好
        // 反过来。实测 配套.pdf 第 11 页缩 0.044, 压过 x_tol(0.025), 于是每一段
        // 都在第一行后面断开: 后半截自成一段, 只剩一两个字的尾行更是单独成段。
        // 断开之后段首那句 "2、质保期内……对于在" 不再以句号收尾, heading_level
        // 就把整段判成二级标题加粗 —— 报上来的"尾行丢了"和"正文被当小标题加粗"
        // 是同一处的两个症状, 那一页七段全中。
        //
        // 只对中文放行: 西文大纲用的是悬挂缩进(续行右移), 那边左移确实是退回
        // 外层 —— Conclusion-for-QA 里 "i. Idempotence…" 后面那条左移 0.061,
        // 跟这里的 0.044 只差一点点, 靠数值分不开, 靠"这段是哪种文字"才分得开。
        //
        // 还得要求这两行合起来横跨大半页: 靠右摆的那几行(页眉"附件2 / 详细技术
        // 描述 / 页码 2 /32" 三行右对齐, 左边界参差 0.03~0.06)左移的幅度跟首行
        // 缩进一模一样, 光看位移分不出来, 得看它们是不是一整幅宽的段落。
        let first_ind = !cur_marker
            && is_zh(&prev.text)
            && (-0.06..=0.0).contains(&(cur.cx0 - prev.cx0))
            && cur.cx0 < prev.rx1 - 0.35;
        let new_block = prev.rx1 <= cfg.full_line            // 上一行没排满 -> 它已结束
            || cur_marker                                    // 新编号/新项目符号
            || end_punct().is_match(&prev.text)              // 上一段已收尾
            || (lang_clear(&cur.text) && is_zh(&cur.text) != is_zh(&prev.text)) // 中英切换
            || (!hang && !first_ind && (cur.cx0 - prev.cx0).abs() > cfg.x_tol) // 缩进层级变了
...
mod tests {
    ...
    /// 中文段落首行缩进两个字, 续行退回左边界 —— 那不是换了缩进层级
    ///
    /// 数字取自 配套.pdf 第 11 页: 首行 0.126 落在缩进上, 续行 0.083 落在左
    /// 边界, 差 0.044 压过 x_tol(0.025), 于是每一段都在第一行后面断开。断开
    /// 之后段首那句不再以句号收尾, heading_level 就把整段判成二级标题加粗,
    /// 最后那行只剩 "期。" 更是单独成段 —— 再低一点的置信度就被当印记删掉了。
    #[test]
    fn chinese_first_line_indent_continuation_merges() {
        let ls = [
            line("2、质保期内，设备在正常使用情况下，售后服务免费提供维修服务及维修用零部件。对于在",
                 0.126, 0.998, 0.423),
            line("保修期内出现设备配件的维修及更换，顺延其保修期。对于设备的主要、重要零部件单独确定保修",
                 0.083, 0.998, 0.443),
            line("期。", 0.076, 0.125, 0.459),
        ];
        let ps = texts(&ls);
        assert_eq!(ps.len(), 1, "首行缩进的三行是同一段: {ps:?}");
        assert!(
            ps[0].ends_with("单独确定保修期。"),
            "只剩一个字的尾行不能丢"
        );
        assert_eq!(heading_level(&ps[0]), None, "并回来就以句号收尾, 不是标题");
    }

    /// 只有一个汉字的尾行也是中文
    ///
    /// is_zh 要两个汉字才算中文, "期。" 于是被判成"不是中文", 撞上"中英切换 ->
    /// 对照的另一半"那条从段落里甩出去。真正的中英对照, 另一半不会只有一个字。
    #[test]
    fn a_one_character_tail_line_is_not_a_language_switch() {
        let ls = [
            line(
                "对于设备的主要、重要零部件单独确定保修",
                0.083,
            ...
            line("期。", 0.083, 0.125, 0.459),
        ];
        assert_eq!(texts(&ls).len(), 1, "一个字的尾行该并回去");
    }

    /// 靠右摆的几行左边界参差, 位移跟首行缩进一样大 —— 但那不是一段
    ///
    /// 3#线 每页右上角那三行页眉("附件2 / 详细技术描述 / 页码 2 /32")左边界
    /// 依次是 0.942 / 0.886 / 0.912, 相邻差 0.03~0.06, 正落在首行缩进的窗口里。
    /// 分得开靠的是"这两行合起来横跨了大半页没有", 不是位移多大。
    #[test]
    fn right_aligned_header_lines_do_not_merge() {
        let ls = [
            line("附件2", 0.942, 1.000, 0.094),
            line("详细技术描述", 0.886, 0.999, 0.109),
            line("页码 2 /32", 0.912, 0.999, 0.126),
        ];
        assert_eq!(texts(&ls).len(), 3, "靠右摆的三行各是一段");
    }
