...
也能只把里面的**表格导成 `.xlsx`**、出一份 **Markdown**，
或者版面一个像素不动、只在底下垫一层隐形文字的**可搜索 PDF**。四种可以任意组合，一次识别一起出。
...
体积和耗时都在同一台 M 系列 Mac 上实测（96 页扫描合同，`--to docx,xlsx`）。
...
```
                      ┌─ 有文字层 ─> 直接取字 ──────────┐      ┌─> Word     手写 OOXML       (默认, 保版式)
                      │   (原生 PDF)   pdfium           │      ├─> Excel    rust_xlsxwriter  (只导表格)
PDF ──渲染──> 页图 ──┤                                 ├─版面重建─┤
    pdfium            └─ 没文字层 ─> 摊平 ─> 摆正 ─> OCR ┘  规则引擎  ├─> Markdown 纯文本, 图另存
                          (扫描件)   背景估计 投影法 PP-OCRv6  + 框线 + 图形  │
                                                                          └─> 可搜索 PDF  原页图 + 隐形文字层
```
...
| **印章/签名/插图照原样留下** | 识别只产出"哪儿有什么字"，一页上不是字的东西到这一步全没了——合同末尾的红章、手签的名字、示意图和 logo，转出来一概不见，丢的偏偏是最要紧的几样。找法是**减法**：页面上的墨迹，减去识别认下的字，减去表格框线，剩下成团的就是它；印章、签名、插图、logo 因此共用一套判据，不必红章靠颜色、签名靠笔画各写一套（那些在灰度扫描件上一样也不成立）。章盖在字上不要紧，挖掉的是字框，章的圈还在。**裁的是彩色页图**，红章不会变成一团灰印子——彩色页只在真有东西要裁时才渲。已经认出来的版面反过来当排除区：框线表里的零碎不算（表本来就还原成表了），页眉页脚那条横幅也不算（正文里已经按页眉丢过一次）。实测 11 页合同抓到 2 个红骑缝章 + 3 张设备照片，96 页技术协议抓到 3 处手写签名，两份都没有误判 |
| **光照不匀先摊平** | 手机拍的纸一边亮一边暗。这最先坏掉的不是"字看不清"——识别模型对明暗相当皮实，实测糊上人造阴影后行数只差一两行；坏的是**框线检测那个全局阈值**：暗那侧的白纸自己就掉到阈值以下，整片被当成墨迹。实测把一页的右侧压到 55% 亮度，阈下像素从 6% 涨到 46%，那页的框线表直接从 1 张变 0 张；摊平后逐格回到基准。做法是按 96 px 的格子取高分位数估出"这块地方的纸有多白"，再照它归一化；整格被印章盖死时靠邻格补回来，印章不会被除成白板。**量到平就原样放过**——110 页平整扫描件上一页没触发，输出逐字节相同 |
...
| **输出可搜索 PDF** | 版面**一个像素不动**，只在页图底下垫一层看不见的文字（`3 Tr`）——归档、法务、发给对方看，要的往往是这个而不是一份重排过的 Word。贴的是识别时那张彩色页图（摆正、摊平之后的），红章还是红的，文字坐标和图天然对得上。中文能搜的关键在字体：嵌一份 CJK 字体要十几 MB，随手拿系统字体嵌进去发出去授权上也站不住，所以走**预定义 CMap 的非嵌入 CID 字体**（Adobe-GB1 + UniGB-UCS2-H，字码就是 UCS-2 码位）加自带的 ToUnicode 表——字形交给阅读器替换，反正不可见；检索和复制只认 ToUnicode，日文韩文一样搜得到。识别失败的页照样贴图进去：归档件缺页比缺文字层严重得多。用 MuPDF（与我们用的 pdfium 是两套独立实现）验过 96 页那份：页数、每页尺寸逐页对得上，96 页全都抽得出字 |
| **输出 Markdown** | 同一套版面判定另出一份 `.md`：标题按编号形态分级、清单带层级、表格出成管道表、图片单独落进同名的 `.assets/` 文件夹用相对路径引（Typora / Obsidian / GitHub 都认这个摆法）。跨页的长表在这儿也接成一张——96 页那份 Word 出 113 张表、Markdown 出 115 张，两条路的续表判定基本一致。转义只挡会把整段读歪的那几个行首记号（`#` `>` `|`）和格子里的竖线，句中的 `*`、`_` 一概不动：中文正文里挨个转义只会长出一堆反斜杠，比偶尔错一次斜体更难读 |
...
输出格式是**四个开关**，可以任意组合：Word / Excel / Markdown / 可搜索 PDF。
还能**逐个文件设**：队列里每一行右侧有个下拉框，默认是「默认·Word」——跟着顶栏那个走，
改顶栏就一起改；单独勾过的那几行则不受影响。一队文件里有的要表格有的要正文时，不用分两趟跑。
...
# 3. 编译
cargo build --release
./target/release/pdf2doc examples/sample_scanned.pdf --to docx,xlsx
```
...
pdf2doc 文件.pdf --to pdf             # 出可搜索 PDF(版面不动, 垫一层隐形文字)
pdf2doc 文件.pdf --to md              # 出 Markdown, 图片放同名的 .assets/
pdf2doc a.pdf b.pdf --to docx,pdf -o 输出目录   # 逗号并列; all = 四种都出
...
pdf2doc 文件.pdf --no-flatten         # 不要摊平不匀的光照
pdf2doc 文件.pdf --no-figures         # 不要保留印章/签名/插图
pdf2doc 文件.pdf --lang en            # 界面/日志/产物里的标记用英文
pdf2doc 韩文件.pdf --ocr-lang ko      # 按韩文识别(缺语言包时先下)
```

`--to` 对本次命令里的所有文件一起生效，可以逗号并列（`docx,md,pdf`），`all` 是四种都出；
要给单个文件另设格式，用图形界面，或者按格式分两条命令跑（识别结果有缓存，第二条不会重新 OCR）。
可搜索 PDF 的文件名会挂个 `-ocr` 后缀，免得跟原件同名。
