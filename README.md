...
```
                                             ┌─> Word  手写 OOXML   (默认, 保版式)
PDF ──渲染──> 页图 ──OCR──> 文字+坐标 ──版面重建─┤
    pdfium      PP-OCRv6 ONNX     规则引擎+框线  └─> Excel rust_xlsxwriter (只导表格)
```

OCR 只能给出「这一行字在哪个坐标」，把它还原成有结构的 Word 才是难点。版面重建做了这些：

| 处理 | 说明 |
|---|---|
| **框线表格还原** | 原件画了框线的表格**照框线还原行列**：缺哪条内部框线，那里在原件里就是一个**合并单元格**，还原出来照样是合并的（Word 里真合并，Excel 里 `merge_range`）。一格里排了三行字也还是一格——按文字间距猜列时它必然被拆成三行，这是扫描件转表格最常见的坏结果。框线用**游程编码的一维形态学开运算**取，厚度按「面积÷长度」判，不看外接框：扫描件总带零点几度歪斜，一条 1500 px 的横线外接框就有七八像素高。汉字竖笔跟框线一样细一样直，靠「两端都搭在横线上」+「不落在 OCR 文字框里」两道判据滤掉 |
...
程序**运行时一次网都不联**：整个运行期依赖树里没有任何网络库（`cargo tree -e normal`
...
- **框线还原受制于原件真有框线**：没画线的相邻表头会并成一格。
- **窄列里的小字可能串格**：OCR 的行检测会把相邻两个窄格的字连成一行。
- OCR 在型号、公差这类**密集数字**上仍可能出错，重要文件建议人工抽查。
- 图形界面**不内置中文字体**（那会让包大好几 MB），启动时从系统里找一个 CJK 字体挂上去；
  极简 Linux 环境里若一个都没有，界面中文会显示成方框，装 `fonts-noto-cjk` 即可。
