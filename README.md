...
PDF ──渲染──> 页图 ──┤                                 ├─版面重建─┤
    pdfium            └─ 没文字层 ─> 摆正 ─> OCR ──────┘  规则引擎  └─> Excel rust_xlsxwriter (只导表格)
                          (扫描件)   投影法  PP-OCRv6      + 框线
```
...
| **扫歪了先摆正** | 扫描件和拍照件几乎都歪着一点，实测手边两份合同是 0.2°~1.1°。一度听着不多，但一页宽 1800 px 的横线两端就差了 31 px——框线检测会把它读成断开的好几截，整张表就散了。转正的角度用**投影法**量：把整页的墨点按候选角度投影到纵轴上，真正水平时同一行的字全落进同一格，投影曲线起伏最大，取起伏最大的那个角。先 0.4° 粗扫再 0.1° 细扫，±3° 以内，不到 0.15° 就不动它。实测一份 11 页的合同，关掉转正时有张 6 行的规格表塌成一段串行的乱序文字（`控制器： 电机： 适用电缆线径： … 交流变频西门子电机+刹车 …`），打开就是规规矩矩的 6 行两列 |
...
pdf2doc 文件.pdf --no-deskew          # 不要自动摆正扫歪的页面
